...
# Mask thresholds (HSV)
mask_lower_threshold = np.array([80, 140, 40])
mask_upper_threshold = np.array([120, 255, 255])
...
class LaneFollower:
	...
	self.image_darknet_sub = rospy.Subscriber('/darknet_ros/detection_image', Image, self.image_callback)
	...
	cv2.imshow('Image', self.lanes_image_pub)
	cv2.waitKey(0)        
	...
	if self.image_darknet_sub.get_num_connections() > 0 :        
	 y = y_expected
	 phi = phi_expected
...
if __name__ == '__main__':
    ...
    # Stop Duckiebot
    cmd_vel = Twist()
    lane_follower.cmd_vel_pub.publish(cmd_vel)
